IMPORTANT COMMOND  :-
 for show all database = show dbs; commond; as = select * from tab;
1 .for enter any database   = use <database ka naam> 
                             
, es commond ka use karne se ager tumhara database bana rahega to bn jayega 
 2 . ager pahle se bana huea hai to us database main enter ho jayega.
  
  2.0 kisi datase ke ander ke sabhi collecion ko dekhne ke liye = show collecion;
 
 3. kisi bhi database ke ander table ya collection banane ke liye 
  = db.createCollection('collection ya table ka naam')
 
 4. collection ko delete karne ke liye  = db.collectionName.drop() 
 aisa karne ke baad ager true ka massage aa jata hai to aapna table ye collection 
 delete ho gaya hai. truncate table <tablename> ki tarah kaam karta hai.
 
 5. kisi bhi database ko delete karne ke liye  = db.dropDatabase();
 command ka use kiya jata hai . drop table <table name > ki tarah kaam karta hai.
 
 6.kisi bi collections ke ander data ko bharne ke liye  = db.<tablename>.insertOne({naem:'sarvesh',price:120})
 
 7. fil kiye gaye data ko dekhen ke liye = db.<tablename>.find();
...
db.procuct.find({},{name:1,_id:1})


Using And and Or Conditions in Mongodb:-
****************************************
Note :: SQL [AND] & [&] clause but mongodb [AND] & [or] the are opreater and every opreator in mongodb is a $keybord hence we have tow , 
$keyword : $and , $or
Eg:-
db.collection.find()
SQL : Select * from collection or table
...
> db.student.find({$and:[{name:"ravi",marks:50}]})
{ "_id" : ObjectId("61ee2d134255 6a6a994e343a"), "roll" : 1001, "name" : "ravi", "marks" : 50 }

> db.student.find({$and:[{name:"ravi",marks:50}]},{roll:1,name:1})
{ "_id" : ObjectId("61ee2d1342556a6a994e343a"), "roll" : 1001, "name" : "ravi" }

NOte :- Your can make a combination of inclion ansd exlusinon in monngodb 6.x
1. either exclude colmun
2.either include columun
3.dont make its commination other wise 


mongodbserverError will be raisedd saying "Projection Donnot Allowed to mx inclusion and exclusion"
eg :-
db.student.find({},{_id:0,name:0,pric:0})=> valid
...
************************************
we use db.collecion .remove {{coditions},true}|false}
2nd Argument : true or false budefault false 
db.collecion.remove {{condition}}
 => n =5 => delete  ==> 5
 
 or 
  db.collecion.remove{{condition},false} => n=5  => delete => 5
  
  db.collecion.remove{{condition},false} => n=5  => delete => 5 => top most
  
  db.collecion.remove {{conditon,false}}
  db.collecion.remove {{conditon,true}}
  db.collecion.remove {{conditon}}
  
  
  
SQL = UPDATE TABLE SET  COLMUN =VALUE , VOLUMN=VALUE, COLUMN=VALUE
...
where clase = db.collections.find() ke barabar hoti hi.
as
IN sql , update is type of query, data level or record query update are of two types
1.single update (withe where clause)
...
SQL :- selelct * from table order by <colomun-name> Asc|desc;
Asc(a)=>1
desc(d)=> -1


NOte :- by deralut , it will sorted by acceding with ref to key (column)
...
db.collection,find(),sort({key:-1}) => decending order.

vvip *****************
SQL :- iNDEX => 0,1,2,3,4,5 ............................n are not same as array.
...
SQL:- PRIMARY KEY , unique or index key index => 
1.clustered
2.non-clustered


index they have there own , Data sturcture areeof three type 
***************************************************************
1.b tree (slowest)
2. b + tree (medium)
3. hastree (fastest)
